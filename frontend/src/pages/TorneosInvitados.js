...
const torneoFinalizado = torneoActual ? new Date(torneoActual.fecha_fin) < new Date() : false;
 ...
 useEffect(() => {
  console.log('[UI] torneoActual.id_torneo:', torneoActual?.id_torneo);
}, [torneoActual]);
              ...
              {loadingPlayoff ? (
                ...
                <div className="bracket-grid">
                  {['OCTAVOS','CUARTOS','SEMIS','FINAL']
                    ...
                    .map(ronda => (
                      <div key={ronda} className="bracket-col">
                        <h4 className="bracket-title">{ronda}</h4>
                        <div className="bracket-col-inner">
                          {playoff[ronda].map(match => {
                            const ganador1 = match.ganador_id && match.ganador_id === match.equipo1_id;
                            const ganador2 = match.ganador_id && match.ganador_id === match.equipo2_id;
                            const setsCargados = match.set1_equipo1 !== null && match.set1_equipo2 !== null;

                            return (
                              <div key={match.id} className={`bracket-card ${match.estado}`}>
                                <div className={`team ${ganador1 ? 'win' : ''}`}>
                                  <span className="name">{match.equipo1_nombre || '—'}</span>
                                  {setsCargados && (
                                    <span className="sets">
                                ...
                                <div className={`team ${ganador2 ? 'win' : ''}`}>
                                  <span className="name">{match.equipo2_nombre || '—'}</span>
                                  {setsCargados && (
                                    <span className="sets">
                                ...
                                {!setsCargados && (
                                  <div className="status">{match.estado.replace('_', ' ')}</div>
                                )}
                              </div>
                            );
                          })}
